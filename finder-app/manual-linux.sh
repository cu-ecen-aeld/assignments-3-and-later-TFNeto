...
mkdir -p ${OUTDIR}/rootfs/{bin,dev,etc,home/conf,lib,lib64,proc,sbin,sys,tmp,usr/{bin,sbin,lib},var/log}
...
# copy the busybox binary to the rootfs and create the necessary links
make CONFIG_PREFIX=${OUTDIR}/rootfs install arch=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} install
...
#get the path of the cross compiler
CROSS_COMPILE_FULLPATH=$(dirname $(which ${CROSS_COMPILE}gcc))
...
# ensure that the root directory is owned by root
